...
class Page < ActiveRecord::Base
  ...
  has_many :sidebars, :through => :page_partials, :foreign_key => :child_page_id
  accepts_nested_attributes_for :page_partials, :allow_destroy => true
  has_many :page_footers
  has_many :footers, :through => :page_footers, :foreign_key => :footer_id
  accepts_nested_attributes_for :page_footers, :allow_destroy => true
  ...
  scope :partials, where(:is_partial => true)
  scope :footers, where(:is_footer => true)
  ...
  scope :in_menu, lambda {
    where(:show_in_menu => true, :is_partial => false, :is_footer => false).includes(:translations).where(
  ...
  def title_with_meta
    ...
    title << "<em>(#{::I18n.t('partial', :scope => 'admin.pages.page')})</em>" if is_partial
    title << "<em>(#{::I18n.t('footer', :scope => 'admin.pages.page')})</em>" if is_footer
    title.join(' ')
  end
