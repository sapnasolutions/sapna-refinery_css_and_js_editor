...
this.itemcontainer="<"+containerstr.toLowerCase()+" class='job'>"
}
...
//		var itemtitle="<a href=\"" + feeds[i].link + "\" target=\"" + this.linktarget + "\" class=\"titlefield\">" + feeds[i].title + "</a>"
		var itemtitle = "<h2 class='job-title'>"+feeds[i].title+"<a target='_blank' class='learn-more-right' href='"+feeds[i].link+"'>Apply Now »</a></h2>"
		...
		itemdescription=itemdescription.replace(/\&lt\;/gi, '<')
		itemdescription=itemdescription.replace(/\&gt\;/gi, '>')
		rssoutput+=this.itemcontainer + itemtitle + " " + itemlabel + " " + itemdate + "\n" + itemdescription + this.itemcontainer.replace("<", "</") + "\n\n"
	}
	rssoutput+=(this.itemcontainer=="<li>")? "</ul>" : ""
	this.feedcontainer.innerHTML=rssoutput
	$('.job>div').addClass('desc')
	$('.desc').hide();
	$('.job-title').click(function(){
		$(this).parents('.job').children('.desc').toggle();
	})
}
